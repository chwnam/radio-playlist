 ...
 *
 * abstracts/registers/abstract-rapl-register-base-rewrite-rule.php
 */
...
if ( ! class_exists( 'RAPL_Register_Base_Rewrite_Rule' ) ) {
	abstract class RAPL_Register_Base_Rewrite_Rule implements RAPL_Register {
		use RAPL_Hook_Impl;
		...
		public function register(): void {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof RAPL_Reg_Rewrite_Rule ) {
					$item->register();
					if ( $item->binding ) {
		...
		public function handle_binding(): void {
			...
			if ( ( $binding = $this->bindings[ $wp->matched_rule ] ?? null ) ) {
				try {
					$callback = RAPL_Main::get_instance()->parse_callback( $binding );
					...
					wp_die(
						new WP_Error(
							...
							sprintf( __( 'Rewrite rule binding `%s` is invalid. Please check your rewrite rule register items.', 'rapl' ), rapl_format_callable( $binding ) )
						)
					);
				...
				if ( apply_filters( 'rapl_register_rewrite_rule_exit', true, $wp->matched_rule ) ) {
					exit;
				}
			}
		}
