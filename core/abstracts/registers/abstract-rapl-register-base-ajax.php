 ...
 *
 * abstracts/registers/abstract-rapl-register-base-ajax.php
 */
...
if ( ! class_exists( 'RAPL_Register_Base_AJAX' ) ) {
	abstract class RAPL_Register_Base_AJAX implements RAPL_Register {
		...
		public function dispatch(): void {
			...
			if ( $action && isset( $this->inner_handlers[ $action ] ) ) {
				try {
					$callback = RAPL_Main::get_instance()->parse_callback( $this->inner_handlers[ $action ] );
			...
			if ( $autobind ) {
				$dispatch = function () use ( $autobind ) {
					...
					if ( ! $autobind['exempt_nonce'] ) {
						check_ajax_referer( $autobind['nonce_action'], '_rapl_nonce' );
					}
					$autobind['callback']();
				};
		...
		public function is_autobind_enabled(): bool {
			return apply_filters( 'rapl_ajax_autobind_enabled', $this->autobind );
		}
	}
}
