 ...
 *
 * abstracts/registers/abstract-rapl-register-base-shortcode.php
 */
...
if ( ! class_exists( 'RAPL_Register_Base_Shortcode' ) ) {
	abstract class RAPL_Register_Base_Shortcode implements RAPL_Register {
		use RAPL_Hook_Impl;
		...
		public function register(): void {
			foreach ( $this->get_items() as $item ) {
				if ( $item instanceof RAPL_Reg_Shortcode ) {
		...
		public function heading_actions_handler(): void {
			if ( $this->heading_actions && is_singular() ) {
				...
				foreach ( array_unique( $this->found_tags ) as $tag_info ) {
					...
					$callback = RAPL_Main::get_instance()->parse_callback( $this->heading_actions[ $tag ] );
		...
		public function dispatch( array|string $atts, string $enclosed, string $tag ): string {
			$callback = RAPL_Main::get_instance()->parse_callback( $this->real_callbacks[ $tag ] ?? '__return_empty_string' );
