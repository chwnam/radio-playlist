 ...
 *
 * traits/trait-rapl-hook-impl.php
 */
...
if ( ! trait_exists( 'RAPL_Hook_Impl' ) ) {
	trait RAPL_Hook_Impl {
		/**
		 * Modified add_action().
		 ...
		 * @param int|null                  $priority         If null, priority comes from 'RAPL_PRIORITY' constant.
		...
		private function hook_get_priority( ?int $priority ): int {
			return is_null( $priority ) ? rapl_priority() : $priority;
		}
	}
}
