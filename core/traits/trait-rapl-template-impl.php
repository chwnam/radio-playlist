 ...
 *
 * traits/trait-rapl-template-impl.php
 */
...
if ( ! trait_exists( 'RAPL_Template_Impl' ) ) {
	trait RAPL_Template_Impl {
		/**
		 * Current block key.
		 */
		...
		protected function locate_file(
			...
			$cache      = RAPL_Main_Base::get_instance()->get( 'rapl:locate_file', [] );
			...
			if ( isset( $cache[ $cache_name ] ) ) {
				...
				$plug = rapl_is_plugin() ? dirname( rapl_main_file() ) : false;
				...
				if ( rapl_is_theme() ) {
					...
					$paths = [
						$variant ? "$styl/rapl/$dir/$file_name-$variant.$ext" : false,
						"$styl/rapl/$dir/$file_name.$ext",
						$variant ? "$tmpl/rapl/$dir/$file_name-$variant.$ext" : false,
						"$tmpl/rapl/$dir}/$file_name.$ext",
						$plug && $variant ? "$plug/includes/templates/$dir/$file_name-$variant.$ext" : false,
						$plug ? "$plug/includes/templates/$dir/$file_name.$ext" : false,
					];
				}

				$paths   = apply_filters( 'rapl_locate_file_paths', array_filter( $paths ), $cache_name );
				$located = false;
				...
				$located = apply_filters( 'rapl_located_path', $located, $tmpl_type, $relpath, $variant, $ext );

				$cache[ $cache_name ] = $located;

				RAPL_Main_Base::get_instance()->set( 'rapl:locate_file', $cache );
			}
		...
		protected function enqueue_ejs( string $relpath, array $context = [], string $variant = '' ): self {
			$ejs_queue = RAPL_Main_Base::get_instance()->get( 'rapl:ejs_queue' );

			if ( ! $ejs_queue ) {
				$ejs_queue = new RAPL_EJS_Queue();
				RAPL_Main_Base::get_instance()->set( 'rapl:ejs_queue', $ejs_queue );
			}
		 ...
		 */
		protected function script( string $handle ): RAPL_Script_Helper {
			return new RAPL_Script_Helper( $this, $handle );
		}
		 ...
		 */
		protected function style( string $handle ): RAPL_Style_Helper {
			return new RAPL_Style_Helper( $this, $handle );
		}
