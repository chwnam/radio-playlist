 ...
 *
 * traits/trait-rapl-autobind-impl.php
 */
...
if ( ! trait_exists( 'RAPL_Autobind_Impl' ) ) {
	trait RAPL_Autobind_Impl {
		/**
		 ...
		 *
		 * Rule:
		 * - {rapl}:{module_notation}@{callback}?foo&bar
		 * - {rapl}:{module_notation}@{callback}!
		 *
		 * NONCE:
		...
		public function parse_autobind(): array|false {
			...
			$regex  = '/^rapl:(([A-Za-z0-9_.]+)@)?([A-Za-z0-9_]+)(!|\?([A-Za-z0-9\-_&]*))?(\*?)$/';
			...
			try {
				$module   = $matches[2] ?? '';
				$func     = $matches[3] ?? '';
				$callback = rapl_parse_callback( $module ? "$module@$func" : $func );
			} catch ( RAPL_Callback_Exception $e ) {
				wp_die( $e->getMessage() );
			}

			$nonce_action = '_rapl_nonce';
