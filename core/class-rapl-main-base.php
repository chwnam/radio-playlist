...
if ( ! class_exists( 'RAPL_Main_Base' ) ) {
	/**
	 * Class RAPL_Main_Base
	 */
	class RAPL_Main_Base implements RAPL_Module {
		use RAPL_Hook_Impl;
		 ...
		 * Singleton instance
		 */
		protected static ?RAPL_Main_Base $instance = null;
		...
		public static function set_instance( RAPL_Main_Base $instance ): void {
			static::$instance = $instance;
			static::$instance->initialize();
		}

		/**
		 * RAPL_Main_Base constructor.
		 */
		...
		public function __sleep() {
			throw new Exception( 'RAPL_Main does not support object serialization.' );
		}
		...
		public function parse_callback( Closure|array|string $item ): Closure|array|string {
			...
			throw new RAPL_Callback_Exception(
				sprintf(
				/* translators: formatted module name. */
					__( '%s is invalid for callback.', 'rapl' ),
					rapl_format_callable( $item )
				)
			);
		}
		...
		public function load_textdomain(): void {
			if ( rapl_is_theme() ) {
				...
				load_plugin_textdomain(
					'rapl',
					false,
					wp_basename( dirname( rapl_main_file() ) ) . '/languages'
				);
			}
		}
		...
		public function activation(): void {
			do_action( 'rapl_activation' );
		}

		public function deactivation(): void {
			do_action( 'rapl_deactivation' );
		}
		 ...
		 *
		 * @uses RAPL_Main_Base::init_conditional_modules()
		 * @uses RAPL_Main_Base::load_textdomain()
		 */
		protected function initialize(): void {
			$this
				 ...
				 */
				->add_action( 'init', 'assign_later_modules', rapl_priority() + 10 )
			;

			if ( rapl_is_theme() ) {
				$this->add_action( 'after_setup_theme', 'load_textdomain', rapl_priority() + 10 );
			} else {
				$this->add_action( 'plugins_loaded', 'load_textdomain', rapl_priority() + 10 );
			}
		...
		protected function setup_activation_deactivation(): self {
			if ( rapl_is_theme() ) {
				...
				$file = plugin_basename( rapl_main_file() );
				$this
		 ...
		 * @return $this
		 */
		protected function assign_constructors( array $constructors ): RAPL_Main_Base {
		...
		/**
		 * Do RAPL_Main specific initialization.
		 */
