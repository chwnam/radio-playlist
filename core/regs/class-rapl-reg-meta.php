...
if ( ! class_exists( 'RAPL_Reg_Meta' ) ) {
	 ...
	 */
	class RAPL_Reg_Meta implements RAPL_Reg {
			...
			if ( isset( $wp_meta_keys[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
				...
				if ( ! isset( static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] ) ) {
					static::$meta[ $object_type ][ $object_subtype ][ $meta_key ] =
						new RAPL_Reg_Meta( $object_type, $meta_key, $args );
				}
			...
			if ( $this->object_type && $this->get_key() ) {
				try {
					if ( $this->args['sanitize_callback'] ) {
						$this->args['sanitize_callback'] = RAPL_Main::get_instance()->parse_callback( $this->args['sanitize_callback'] );
					}
					...
					$error->add(
						...
						sprintf(
							...
							rapl_format_callable( $this->args['sanitize_callback'] )
						)
					);
				...
				try {
					if ( $this->args['auth_callback'] ) {
						$this->args['auth_callback'] = RAPL_Main::get_instance()->parse_callback( $this->args['auth_callback'] );
					}
				} catch ( RAPL_Callback_Exception $e ) {
					$error = new WP_Error();
					$error->add(
						...
						sprintf(
							...
							rapl_format_callable( $this->args['auth_callback'] )
						)
					);
					// $error is a WP_Error instance.
					// phpcs:ignore WordPress.Security.EscapeOutput
