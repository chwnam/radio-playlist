 ...
 *
 * regs/class-rapl-reg-script.php
 */
...
if ( ! class_exists( 'RAPL_Reg_Script' ) ) {
	class RAPL_Reg_Script implements RAPL_Reg {
		public const WP_SCRIPT = 'rapl-wp-script-generated';
			...
			if ( $this->handle && $this->src && ! wp_script_is( $this->handle, 'registered' ) ) {
				if ( self::WP_SCRIPT === $this->deps ) {
					...
					if ( $is_theme ) {
						...
						$root = plugin_dir_url( rapl_main_file() ) . 'assets/js/';
					}
					...
					if ( $is_theme ) {
						...
						$path = path_join( dirname( rapl_main_file() ), "assets/js/$dir/$file" );
					}
					...
					if ( $is_theme ) {
						...
						$this->src = plugins_url( "assets/js/$this->src", rapl_main_file() );
					}

					$this->deps      = $info['dependencies'] ?? [];
					$this->ver       = $info['version'] ?? rapl_version();
					$this->in_footer = true;
				}
		 ...
		 *
		 * @return RAPL_Reg_Script
		 */
		public static function wp_script( string $handle, string $src ): RAPL_Reg_Script {
			return new RAPL_Reg_Script( $handle, $src, self::WP_SCRIPT );
		}
	}
}
