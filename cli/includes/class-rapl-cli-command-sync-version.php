...
if ( ! class_exists( 'RAPL_CLI_Command_Sync_Version' ) ) {
	final class RAPL_CLI_Command_Sync_Version implements RAPL_CLI_Command {
		...
		public function run( Console_CommandLine_Result $parsed ): void {
			...
			if ( $parsed->options['main_file'] ) {
				...
				$result    = RAPL_CLI_Header_Detect::get_header_data( $main_file );
			} else {
				// Discover.
				$result    = RAPL_CLI_Header_Detect::detect( RAPL_ROOT );
			...
			if ( RAPL_CLI_Header_Detect::TYPE_THEME === $result['type'] && $main_file ) {
			...
			$slug_data = rapl_cli_get_dot_slug();
			$slug      = $slug_data['slug'] ?? rapl_cli_the_slug();
			...
			echo '* Main file: ' . substr( $main_file, strlen( RAPL_ROOT ) + 1 ) . PHP_EOL;
			...
			$files = [
				$main_file,
				$functions_file,
				RAPL_ROOT . '/composer.json',
				RAPL_ROOT . '/package.json',
			];
		...
		public function apply_change_php(
			...
			$extractor = new RAPL_CLI_Token_Extract( new RAPL_CLI_Node_Visitor_Const() );
			...
			if ( $content ) {
				 ...
				 *
				 * @var RAPL_CLI_Token_Const[] $tokens
				 */
		...
		public function apply_change_json( string $json_path, string $target_version ): void {
			...
			if ( is_array( $content ) ) {
				...
				if ( $version !== $target_version ) {
					$file_name = substr( $json_path, strlen( RAPL_ROOT ) + 1 );
		...
		private function replace_header_version( string $content, string $slug, string $target_version ): string {
			if ( $content ) {
				...
				[ $value, $offset ] = RAPL_CLI_Header_Detect::extract_header( $content, 'Version' );
				...
				[ $value, $offset ] = RAPL_CLI_Header_Detect::extract_header( $content, 'CPBN Version' );
				if ( rapl_cli_the_slug() === $slug && $offset && $value !== $target_version ) {
