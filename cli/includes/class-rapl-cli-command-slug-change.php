<?php
if ( ! class_exists( 'RAPL_CLI_Command_Slug_Change' ) ) {
	final class RAPL_CLI_Command_Slug_Change implements RAPL_CLI_Command {
		private RAPL_CLI_Regex_Replace $regex_replace;

		private RAPL_CLI_Textdomain_Replace $textdomain_replace;
		...
		private string $root_dir = RAPL_ROOT;
		...
		public function initialize( Console_CommandLine_Result $parsed ): void {
			do {
				...
				if ( rapl_cli_the_slug() !== $this->old_slug ) {
					self::validate_slug( $this->old_slug );
				}
				self::validate_slug( $this->new_slug );

				if ( rapl_cli_the_slug() !== $this->old_textdomain ) {
			...
			$this->regex_replace      = new RAPL_CLI_Regex_Replace( $this->old_slug, $this->new_slug );
			$this->textdomain_replace = new RAPL_CLI_Textdomain_Replace( $this->old_textdomain, $this->new_textdomain );
		}
		...
		private function slug_prompt(): string {
			do {
				$confirmed = false;
				$new_slug  = rapl_cli_prompt( 'Please enter your new slug (Enter \'exit\' to skip):' );
				if ( 'exit' === $new_slug ) {
