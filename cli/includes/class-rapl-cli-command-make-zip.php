<?php
if ( ! class_exists( 'RAPL_CLI_Command_Make_Zip' ) ) {
	final class RAPL_CLI_Command_Make_Zip implements RAPL_CLI_Command {
		...
		public function run( Console_CommandLine_Result $parsed ): void {
			...
			if ( empty( $output ) ) {
				$output = dirname( $project_dir ) . '/' . rapl_cli_basename( $project_dir ) . '.zip';
			}
			...
			$zip->addEmptyDir( rapl_cli_basename( $project_dir ) );
