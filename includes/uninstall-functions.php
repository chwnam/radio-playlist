...
if ( ! function_exists( 'rapl_cleanup_meta' ) ) {
	/**
	 * Remove plugin's meta records.
	 */
	function rapl_cleanup_meta(): void {
		global $wpdb;
		...
		foreach ( array_keys( $meta_keys ) as $key ) {
			$meta = rapl()->registers->{$key . '_meta'};
			if ( $meta ) {
				foreach ( $meta->get_items() as $item ) {
					if ( $item instanceof RAPL_Reg_Meta ) {
...
if ( ! function_exists( 'rapl_cleanup_option' ) ) {
	/**
	 * Remove plugin's option records.
	 */
	function rapl_cleanup_option(): void {
		global $wpdb;

		$option_names = [];

		$option = rapl()->registers->option;
		if ( $option ) {
			foreach ( $option->get_items() as $item ) {
				if ( $item instanceof RAPL_Reg_Option ) {
...
if ( ! function_exists( 'rapl_cleanup_terms' ) ) {
	/**
	 * Remove plugin's terms.
	 */
	function rapl_cleanup_terms(): void {
		global $wpdb;

		$taxonomies = [];

		$taxonomy = rapl()->registers->taxonomy;
		if ( $taxonomy ) {
...
if ( ! function_exists( 'rapl_cleanup_posts' ) ) {
	/**
	 * Remove plugin's posts.
	 */
	function rapl_cleanup_posts(): void {
		global $wpdb;

		$post_types = [];

		$post_type = rapl()->registers->post_type;
		if ( $post_type ) {
