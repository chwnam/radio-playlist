...
if ( ! class_exists( 'RAPL_Register_Theme_Support' ) ) {
	class RAPL_Register_Theme_Support extends RAPL_Register_Base_Theme_Support {
		...
		public function get_items(): Generator {
			yield new RAPL_Reg_Theme_Support( 'automatic-feed-links' );

			yield new RAPL_Reg_Theme_Support( 'post-thumbnails' );

			yield new RAPL_Reg_Theme_Support(
				'post-formatst',
				[
					'aside',
					'gallery',
			...
			yield new RAPL_Reg_Theme_Support( 'title-tag' );
		}
		...
		public function map_front_modules( WP_Query $query ) {
			...
			*/
			...
			if ( $hierarchy->is_archive() ) {
				...
				if ( $module instanceof RAPL_Front_Archive_Module ) {
		...
		protected function extra_register(): void {
			...
			register_nav_menus(
				[
					'primary'   => __( 'Primary Menu', 'rapl' ),
					'secondary' => __( 'Secondary Menu', 'rapl' ),
				]
			);

			register_sidebar(
				[
					'name'          => __( 'Primary Sidebar', 'rapl' ),
					'id'            => 'sidebar-1',
			...
			register_sidebar(
				[
					'name'          => __( 'Secondary Sidebar', 'rapl' ),
					'id'            => 'sidebar-2',
			...
			/** @see RAPL_Main::load_textdomain() for textdomain loading. */
		}
	}
}
